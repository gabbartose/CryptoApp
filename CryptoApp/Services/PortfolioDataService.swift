...
class PortfolioDataService {
    ...
    // MARK: - Public
    /// This function determin whether we are adding, updating or deleting those coins from Portfolio
    func updatePortfolio(coinModel: CoinModel, amount: Double) {
        // check if coin is already in Portfolio
        if let entity = savedEntities.first(where: { $0.coinID == coinModel.id }) {
            if amount > 0 {
    ...
    // MARK: - Private functions for Core Data manipulation
